...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        # Three lateral 1x1 conv layers to transform c3, c4, and c5 to standardize the channel dimension
        self.fpn_params["c3"] = nn.Conv2d(
            dummy_out_shapes[0][1][1], self.out_channels, 1, 1, 0
        )
        self.fpn_params["c4"] = nn.Conv2d(
            dummy_out_shapes[1][1][1], self.out_channels, 1, 1, 0
        )
        self.fpn_params["c5"] = nn.Conv2d(
            dummy_out_shapes[2][1][1], self.out_channels, 1, 1, 0
        )

        # Create 3x3 conv layers to ouput p3, p4, and p5 features
    ...
    def forward(self, images: torch.Tensor):
        ...
        # pass c5 through self.fpn_params["c5"] to get p5
        p5 = self.fpn_params["c5"](backbone_feats["c5"])

        # p4 -> pass c4 through self.fpn_params["c4"], interpolate p5 and element-wise addition
        p4 = self.fpn_params["c4"](backbone_feats["c4"]) + F.interpolate(
            p5, backbone_feats["c4"].shape[2], mode="nearest"
        )

        # p3 -> pass c3 through self.fpn_params["c3"], interpolate p4 and element-wise addition
        p3 = self.fpn_params["c3"](backbone_feats["c3"]) + F.interpolate(
            p4, backbone_feats["c3"].shape[2], mode="nearest"
        )

        # 3x3 conv
        fpn_feats["p3"] = self.fpn_params["p3"](p3)
        fpn_feats["p4"] = self.fpn_params["p4"](p4)
        fpn_feats["p5"] = self.fpn_params["p5"](p5)
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(self, num_classes: int, in_channels: int, stem_channels: List[int]):
        ...
        self.pred_cls = nn.Conv2d(in_channels, num_classes, 3, 1, 1)
        # Box regression conv
        self.pred_box = nn.Conv2d(in_channels, 4, 3, 1, 1)
        # Centerness conv
        self.pred_ctr = nn.Conv2d(in_channels, 1, 3, 1, 1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, value in feats_per_fpn_level.items():
            ...
            stem_cls_output = self.stem_cls(value)
            stem_box_output = self.stem_box(value)

            # flatten to dimension (B, H * W, C)

            class_logits[key] = (
                self.pred_cls(stem_cls_output).flatten(start_dim=2).permute(0, 2, 1)
            )
            boxreg_deltas[key] = (
                self.pred_box(stem_box_output).flatten(start_dim=2).permute(0, 2, 1)
            )
            centerness_logits[key] = (
                self.pred_ctr(stem_box_output).flatten(start_dim=2).permute(0, 2, 1)
            )
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for i in range(gt_boxes.shape[0]):
            matched_deltas_dict = {}
            for fpn_level, matched_boxes in matched_gt_boxes[i].items():
                if len(matched_boxes) > 0:
                    matched_deltas_dict[fpn_level] = fcos_get_deltas_from_locations(
                        locations_per_fpn_level[fpn_level],
                        matched_boxes,
                        self.backbone.fpn_strides[fpn_level],
                    )
                else:
                    matched_deltas_dict[fpn_level] = torch.empty(0, 4)
        ...
        for img, loc in zip(imgs, locs):
            onehotvectors_gt[img, loc, gt_cls_idx[img, loc]] = 1

        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=onehotvectors_gt)
